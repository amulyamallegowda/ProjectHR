...
public class DataAccess {
	...
	public List<Object> getTrainingsToApprove()
	{
		...
		try {
			...
			while(result.next()){
				...
				if(result.getString("Status").equals("Completed"))
					updateEmpSkill(result.getInt("TraineeID"),result.getInt("SkillID"));
			}
			result.close();
			statement.close();
		}
	...
	public HashMap<String,Object> getAppraisalsOfDepartment(int deptID)
	{
		...
		try {
			...
			statement = sqlConnection.prepareStatement("EXEC [dbo].[GetAppraisalTable] ?");
			...
			while(result.next()){
				dataset.put("EmpID", result.getInt("EmpID"));
				dataset.put("ManagerID", result.getInt("ManagerID"));
	...
	public int getEmployeeID(int employeeID) {		
		PreparedStatement preprdStmt = null;
		String selectquery = "SELECT 1 FROM [ProjectHR].[dbo].[Employee] WHERE ID = ?";
	...
	public void getEmployeeDetails(int employee_id) {
		...
		try {
			...
			if (rs.next()) {
				...
				System.out.println("Emergency Contact number 	: " + rs.getString("EmergencyContactNumber"));
	...
	private boolean updateEmpSkill(int ID, int SkillId){
		try{
